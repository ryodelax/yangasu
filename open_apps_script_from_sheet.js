...
(async () => {
  ...
  console.log('BODY1', txt.replace(/\n/g,' | '));
  const found = await page.evaluate(() => {
    const els = [...document.querySelectorAll('*')].filter(el => /Apps Script|Apps Script/.test((el.textContent||'').trim()) && (el.offsetWidth || el.offsetHeight || el.getClientRects().length));
    return els.map(el => ({text:(el.textContent||'').trim(), tag:el.tagName, cls:el.className, role:el.getAttribute('role')})).slice(0,20);
  });
  console.log(JSON.stringify(found,null,2));
  await page.screenshot({path:'/tmp/open_apps_script_from_sheet.png', fullPage:false});
  await browser.close();
