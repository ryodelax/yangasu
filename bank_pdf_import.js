...
function renderOperationalConquestView_(ss) {
  const rows = buildOperationalConquestViewContexts_(ss);
  const rollups = buildOperationalConquestRollups_(rows);
  const sheet = ensureOperationalViewSheet_(ss, 'コンクエスト管理', 10);
  ...
  writeOperationalSnapshot_(sheet, values, 10);
  applyOperationalSheetDecorations_(sheet, {
    title: 'コンクエスト管理',
    headerRow: 1,
    dataStartRow: 2,
...
function buildOperationalSalesBaseContexts_(ss) {
  ...
  const rows = values.map(function(row) {
    ...
    const specialBucket = resolveOperationalSpecialBucket_(billTo, specialType);
    return {
...
function resolveOperationalSpecialBucket_(billTo, specialType) {
  const normalizedBillTo = normalizeMatchText_(billTo);
  const normalizedSpecial = normalizeMatchText_(specialType);
  if (normalizedSpecial.indexOf(normalizeMatchText_('コンクエスト')) >= 0 || normalizedBillTo.indexOf(normalizeMatchText_('コンクエスト')) >= 0) {
    return 'conquest';
  }
  const isInsurance = SPECIAL_CASE_CFG.INSURANCE_HINTS.some(function(keyword) {
    return normalizedBillTo.indexOf(normalizeMatchText_(keyword)) >= 0;
  });
...
function importBankPdfFilesToSheet() {
  ...
  try {
    ...
    const spreadsheet = SpreadsheetApp.openById(BANK_PDF_IMPORT.spreadsheetId);
