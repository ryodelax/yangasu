...
function onOpen() {
  ...
  SpreadsheetApp.getUi()
    .createMenu('データ取り込み')
    .addItem('CSVから取り込み', 'runDailyImport')
...
function buildStage1RecordFromCsvRow_(row, csvHeaderMap) {
  return {
    '整備No': getCsvValueByAliases_(row, csvHeaderMap, ['整備No', '整備№', '整備ナンバー', '整備番号']),
    '日付': getCsvValueByAliases_(row, csvHeaderMap, ['日付', '見積日', '受付日']),
    ...
    '作業大区分': getCsvValueByAliases_(row, csvHeaderMap, ['作業大区分', '作業区分', '大区分']),
    '整備店舗': getCsvValueByAliases_(row, csvHeaderMap, ['整備店舗', '管理店舗', '営業店舗', '店舗', '店名', '作業小区分名']),
    '請求先名': getCsvValueByAliases_(row, csvHeaderMap, ['請求先名', '請求先']),
    '状況': getCsvValueByAliases_(row, csvHeaderMap, ['状況', '進捗', 'ステータス']),
    '売上総計': getCsvNumberByAliases_(row, csvHeaderMap, ['売上総計', '売上合計', '売上計']),
    '粗利益': getCsvNumberByAliases_(row, csvHeaderMap, ['粗利益'])
  };
}
...
function getStage1CsvHeaderAliases_() {
  return {
    ...
    '納車日': ['納車日'],
    '車検日': ['車検日', '検査満了日'],
    '作業大区分': ['作業大区分', '作業区分', '大区分'],
    '整備店舗': ['整備店舗', '管理店舗', '営業店舗', '店舗', '店名', '作業小区分名'],
    '請求先名': ['請求先名', '請求先'],
    '状況': ['状況', '進捗', 'ステータス'],
    '売上総計': ['売上総計', '売上合計', '売上計'],
    '粗利益': ['粗利益']
...
function getCsvValueByAliases_(row, csvHeaderMap, aliases) {
  for (let i = 0; i < (aliases || []).length; i++) {
    ...
    if (index !== undefined) {
      return normalizeCsvCellValue_(row[index]);
    }
  }
  return '';
}
...
function distributeServiceData() {
  ...
  if (rowsSyaken.length) {
    ...
    applyStage2StatusColors_(sheetSyaken, headersSyaken, 2);
  }
  ...
  if (rows12ten.length) {
    ...
    applyStage2StatusColors_(sheet12ten, headers12ten, 2);
  }
}
