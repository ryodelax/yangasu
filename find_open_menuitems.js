...
(async () => {
  ...
  const found = await page.evaluate(() => {
    return [...document.querySelectorAll('[role="menuitem"]')]
      ...
      .map(el => ({text:(el.textContent||'').trim(), tag:el.tagName, cls:el.className, x:el.getBoundingClientRect().x, y:el.getBoundingClientRect().y, w:el.getBoundingClientRect().width, h:el.getBoundingClientRect().height}))
      .slice(0,200);
  });
