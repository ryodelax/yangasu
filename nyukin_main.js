...
const CONQUEST_HINTS = ['コンクエスト', 'ｺﾝｸｴｽﾄ'];
...
function menuRunReconcile() {
  try {
    ...
    const msg =
      ...
      '銀行一覧・案件一覧・コンクエスト管理・保険立替管理を更新しました。';
...
function runDiagnose() {
  // 案件リスト更新（請求先名を含む最新データ取得）
  var salesCount = syncSalesListFromSource_();
  Logger.log('案件リスト更新: ' + salesCount + '件');
  ...
  // ビュー更新
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  try { renderOperationalBankListView_(ss);  Logger.log('銀行一覧 更新OK'); } catch(e) { Logger.log('銀行一覧更新エラー: ' + e.message); }
  try { renderOperationalSalesView_(ss);     Logger.log('案件一覧 更新OK'); } catch(e) { Logger.log('案件一覧ビュー更新エラー: ' + e.message); }
  try { renderOperationalConquestView_(ss);  Logger.log('コンクエスト管理 更新OK'); } catch(e) { Logger.log('コンクエスト管理更新エラー: ' + e.message); }
  try { renderOperationalInsuranceView_(ss); Logger.log('保険立替管理 更新OK'); } catch(e) { Logger.log('保険立替管理更新エラー: ' + e.message); }
