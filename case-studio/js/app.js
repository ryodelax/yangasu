...
(() => {
  ...
  const ic = {
    ...
    sparkle: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3l1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8z"/><path d="M19 15l.7 2L22 17.7 19.7 18.4 19 21l-.7-2.6L16 17.7 18.3 17z"/></svg>',
  ...
  const SYSTEM_MINUTES =
    "あなたは優秀な議事録作成アシスタントです。与えられた打ち合わせの文字起こしから、日本語で簡潔で読みやすい議事録をMarkdownで作成してください。" +
    "次の見出し構成にしてください:\n" +
    "## 要約\n（3〜5行で全体の要点）\n" +
    "## 決定事項\n（箇条書き。なければ「特になし」）\n" +
    "## ToDo / ネクストアクション\n（箇条書き。担当者や期限が文中にあれば併記）\n" +
    "## 次回までの論点・宿題\n（箇条書き。なければ省略可）\n" +
  ...
  async function onGenerateMinutes(id, btn) {
    ...
    if (!t) return;
    if (!Settings.getKey()) { toast("先に設定でAPIキーを登録してください"); settingsForm(); return; }
    if (btn) { btn.disabled = true; btn.dataset.busy = "1"; btn.innerHTML = `${ic.sparkle}生成中…`; }
    toast("AIが議事録を生成中…");
    try {
      const md = await generateMinutes(t.content);
      Store.transcripts.update(id, { minutes: md, minutesAt: Date.now() });
      toast("議事録を作成しました");
      render();
    } catch (e) {
      toast("生成に失敗: " + e.message);
      if (btn) { btn.disabled = false; btn.removeAttribute("data-busy"); btn.innerHTML = `${ic.sparkle}議事録`; }
    }
  }
  ...
  function copyMinutes(id) {
    ...
    navigator.clipboard.writeText(t.minutes).then(
      () => toast("議事録をコピーしました"),
      () => toast("コピーに失敗しました")
    );
  }

  /* 最小限のMarkdown→HTML（見出し・箇条書き・太字のみ） */
  ...
  function settingsForm() {
    ...
    openModal(`
      ...
      <div class="modal-foot"><button class="btn" data-close>キャンセル</button><button class="btn btn-primary" id="saveSettings">保存</button></div>`);
    $("#saveSettings").onclick = () => {
      Settings.setKey($("#s-key").value.trim());
      Settings.setModel($("#s-model").value);
      toast("設定を保存しました");
      closeModal();
    };
  }
        ...
        <div class="item">
          <div class="item-head">
            ...
            <div class="item-actions">
              <button class="btn btn-ghost btn-sm" data-minutes-tx="${t.id}" aria-label="議事録を作成">${ic.sparkle}議事録</button>
          ...
          <div class="minutes">
            <div class="minutes-head">
              ${ic.sparkle}<span class="minutes-title">議事録</span>
              <span class="minutes-time">${relTime(t.minutesAt)}</span>
              <div class="item-actions">
                <button class="btn btn-ghost btn-sm" data-copy-min="${t.id}">${ic.copy}コピー</button>
                <button class="btn btn-ghost btn-sm" data-minutes-tx="${t.id}">再生成</button>
              </div>
            </div>
            <div class="minutes-body">${mdToHtml(t.minutes)}</div>
          </div>` : ""}
  ...
  content.addEventListener("click", (e) => {
    ...
    if ((t = el("[data-minutes-tx]"))) { if (!t.dataset.busy) onGenerateMinutes(t.dataset.minutesTx, t); return; }
    if ((t = el("[data-copy-min]"))) { copyMinutes(t.dataset.copyMin); return; }
